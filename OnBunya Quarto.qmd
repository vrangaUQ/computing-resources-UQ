...
![](images/Interactive%20Apps.png){fig-align="center"}
...
![](images/Bunya%20Apps.png){fig-align="center"}
...
![](images/VS%20Code.png){fig-align="center"}
...
![](images/VS%20Code%20Item.png){fig-align="center"}
...
![](images/Jupyter%20Notebook.png){fig-align="center"}
...
![](images/Jupyter%20terminal.png){fig-align="center"}
...
![](images/Jupyter%20terminal%202.png){fig-align="center"}
...
![](images/Jupyter%20terminal%203.png){fig-align="center"}
...
![](images/Py25%202.png){fig-align="center"}
...
![](images/Case%20Study.png){fig-align="center"}

Next, create a new conda environment for our YAML file with the commands.

![](images/Case%20Econ.png){fig-align="center"}
...
![](images/Case%20Econ%202.png){fig-align="center"}
...
Now let’s look at a similar example in Julia. Our example is from from [JuliaAcademy/DataScience: Data Science in Julia course for JuliaAcademy.com, taught by Huda Nassar](https://github.com/JuliaAcademy/DataScience). We can download the files from GitHub with the command

![](images/Julia.png){fig-align="center"}

This site was developed using Julia version 1.6.1. We can add this version to our setup via the command

`juliaup add 1.6.1`

Now we want to configure a jupyter kernel for this version of Julia. The jupyterlab module used by the Jupyter instance provided by OnBunya adds the path to another version of Julia in front of the version we have configured through juliaup, so it is better to use the full path to Julia if we are using that service or if have run module load jupyterlab in our terminal on a virtual desktop.

For a default installation, we would use the command

`~/.juliaup/bin/julia +1.6.1`

As with the default environment we installed early, we will need to add the IJulia package and install the kernel
...
Notice that it has automatically added the version of Julia we use to the name of the kernel. Next, we need to install the packages required by the project. Since our example makes calls from other programming languages, we need to configure Julia to find these compilers. For example, I have created a Conda environment called other-compilers from an environment.yml file containing the following
...
We change directory to the DataScience directory and create a file called `LocalPreferences.toml` containing something like

`[RCall]`

`Rhome = “/home/USERNAME/.conda/envs/other-compilers/lib/R"`

`libR = “/home/USERNAME/.conda/envs/other-compilers/lib/R/lib/libR.so"`

`[PyCall]`

`PYTHON = “/home/USERNAME/.conda/envs/other-compilers/bin/python”`

Then start Julia with the command

`~/.juliaup/bin/julia +1.6.1 --project=@.`

Type “\]“ to enter pkg mode and enter precompile to install the packages (this will take some time). Once this is done, we can start a jupyterlab session and open one of the notebooks using the Jupyter 1.6.1 kernel. This example uses more resources than our previous one, so you may need to allocate more resources to your instance to avoid the kernel crashing when executing commands from the notebooks.

### A simple example using R

We can also produce notebooks using R. We will need a suitable R environment with sufficient compilers and libraries to allow us to install any additional R packages we might require. R is not provided as part of the operating system on Bunya, so we will have to create our own R environment or load R using the module command. In both cases, we will have to use a wrapper script to ensure that the environment is loaded prior to running R. In this example we will use the module r/4.4.2-heavy because it already has many packages installed including IRkernel which we need to interact with Jupyter. We create a wrapper file called `~/apps/wrappers/r_cs3.sh` containing
...
`chmod +x ~/apps/wrappers/r_cs3.sh`

The next step should be to register the kernel using the IRkernel package. Unfortunately, the module uses an apptainer container to run R, which does not have the Jupyter client installed in its path, and we can’t use our usual jupyter client from inside the container, so the IRkernel::installspec command fails. However, other functions from IRkernel will work, so if we manually register the kernel, we are still able to use this version of R from within Jupyter Labs. To do this we access the container and copy the kernelspec to our Jupyter kernel directory

`apptainer exec /sw/containers/local/rcc/rstudio_2024.12.0-467_R-4.4.2.sif \`

`cp -r /usr/lib/R/site-library/IRkernel/kernelspec ~/.local/share/jupyter/kernels/r-cs3`

Then we replace the R command in its kernel.json file with the path to our wrapper script

`sed -i  's#"R", "--slave"#"~/apps/wrappers/r_cs3.sh", "--slave"#' ~/.local/share/jupyter/kernels/r-cs3/kernel.json`

Then we change the display name so that it is unique

`sed -i ‘s# "display_name":"R"# "display_name":"R"#’  ~/.local/share/jupyter/kernels/r-cs3/kernel.json`

Now we should be able to see our new version of are in a jupyterlabs session. To demonstrate this, we can run the code from the [Analyzing Hosptial Patient Data in R](https://www.geeksforgeeks.org/r-machine-learning/analyzing-hospital-patient-data-in-r/) article on the GeeksforGeeks website. Create a new Jupyter notebook and select the new R kernel. Copy the code segments from the website into cells within the notebook and execute the cell using the Shift & Enter keys.

![](images/R%20Jupyter.png){fig-align="center"}
